...
#[pymethods]
impl GraphRefiner {
    /// Create a new graph refiner using the command‑string GA.  `population_size`
    /// controls how many genomes are maintained and `gene_length` sets the
    /// number of encoded commands in each genome.
    #[new]
    pub fn new(population_size: usize) -> Self {
        GraphRefiner {
            optimizer: GeneticOptimizer::new(population_size),
        }
    }
    ...
    pub fn load_initial_graph(&mut self, num_nodes: usize, edge_index: Vec<(usize, usize)>, seed: u64, gene_length: usize) {
        self.optimizer.initialize_population(num_nodes, edge_index, seed, gene_length);
    }
