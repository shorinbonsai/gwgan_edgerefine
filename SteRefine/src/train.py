...
def compute_gradient_penalty(discriminator: nn.Module, real_data: Data, fake_data: Data, class_labels: torch.Tensor, device: torch.device, lambda_gp: float = 10.0):
    ...
    # Interpolated data uses real graph topology, no edge_weight.
    # The discriminator's ew=None fallback handles this correctly.
...
def train_epoch(generator: nn.Module, discriminator: nn.Module, loader: DataLoader, opt_g, opt_d, config, dataset_stats: Tuple[Dict, Dict], device: torch.device, epoch: int = 1):
    ...
    for batch in loader:
        ...
        for _ in range(config.n_critic):
            ...
            # Generator forward — if use_refinement is True, GA runs inside.
            # .detach() on the Data object detaches all tensors including edge_weight,
            # so no generator gradients are produced during discriminator training.
...
def train(generator: nn.Module, discriminator: nn.Module, train_loader: DataLoader, val_loader: DataLoader, opt_g, opt_d, config, labels, dataset_stats: Tuple[Dict, Dict], device: torch.device, logger: logging.Logger, epochs: int = 1):
    # Training loop
    ...
    if config.use_refinement:
        logger.info(f"GA refinement warmup: {config.refinement_warmup_epochs} epochs "
                    f"(GA disabled during warmup, enabled after)")
    else:
        logger.info("GA refinement: DISABLED for this run")

    for epoch in range(1, config.epochs + 1):
        # ------------------------------------------------------------------
        # WARMUP TOGGLE: Enable GA refinement after warmup period
        # ------------------------------------------------------------------
        if config.use_refinement:
            if epoch <= config.refinement_warmup_epochs:
        ...
        # ------------------------------------------------------------------
        # VALIDATION: GA state is left as-is. If the generator was trained
        # with GA refinement this epoch, validation also uses it — the
        # pipeline must be evaluated as it was trained.
        # ------------------------------------------------------------------
        val_score = evaluate(generator, discriminator, labels, val_loader, train_loader, dataset_stats, config, device, logger)

        refinement_status = "GA-ON" if generator.use_refinement else "GA-OFF"
        logger.info(f"Epoch {epoch:02d} [{refinement_status}] | D_loss: {d_loss:.6f} | G_loss: {g_loss:.6f} | Val: {val_score:.6f}")
...
def evaluate(generator: nn.Module, discriminator: nn.Module, labels: Iterable[int], eval_loader: DataLoader, train_loader: DataLoader, dataset_stats: Tuple[Dict, Dict], config, device: torch.device, logger: logging.Logger, file: bool = False):
    ...
    Respects the current state of generator.use_refinement — the caller is
    responsible for setting it appropriately before calling this function.
    When the GA was active during training, it should also be active during
    evaluation so the pipeline is evaluated as it was trained.
    ...
    for class_label in labels:
        ...
        novel_graphs = [h for h in fake_hashes if h not in train_hashes]
